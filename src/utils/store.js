...
export default function withDva(...args) {
  return function CreateNextPage(Component) {
    ...
    ComponentWithDva.getInitialProps = async (props = {}) => {
      ...
      let initValue = {}
      if (isServer) {
        const {cookies} = new Cookies(props.req.headers.cookie)
        if (cookies && cookies.loginData) {
          const loginData = JSON.parse(cookies.loginData)
          initValue = { app: { loginData } }
        }
      }
      const store = getOrCreateStore(initValue);
